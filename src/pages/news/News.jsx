...
export default function News() {
  ...
  const [content, setContent] = useState(DEFAULT_BLOG['news-news'])

  useEffect(() => {
    let cancelled = false
    fetchBlogContent('news-news').then((data) => {
      if (!cancelled && data) setContent({ ...DEFAULT_BLOG['news-news'], ...data })
    })
    return () => {
      cancelled = true
    }
      ...
      {content.isActive !== false ? (
        <PageHero
          ...
          crumbs={[
            ...
            { label: pick(content.heroLabel, content.heroLabelHi) || 'News' },
          ]}
        />
      ) : null}
      <BlogSection />
